...
subplot(4,1,1),plot(time,x),xlabel('Time(s)'),grid,title('original ECG trace')

% design an stop-band FIR 36-order filter for removing powerline nosie @ 60 Hz 
b=fir1(36,[58 62]/(Fs/2),'stop');
filtered_x=filtfilt(b,1,x); % apply the filter in zero-phase mode 
subplot(4,1,2),plot(time,filtered_x),grid,title('FIR stopband-filtered signal (filter order = 36)')
...
subplot(4,1,3),plot(time,filtered_x),grid,title('butterworth IIR stopband-filtered signal')

% design an stop-band FIR 150-order filter for removing powerline nosie @ 60 Hz 
b=fir1(150,[58 62]/(Fs/2),'stop');
filtered_x=filtfilt(b,1,x); % apply the filter in zero-phase mode 
subplot(4,1,4),plot(time,filtered_x),grid,title('FIR stopband-filtered signal (filter order = 150)')
